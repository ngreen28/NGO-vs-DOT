...
namespace spawnPlayer.Samples.SpawnPlayer.Scripts
{
    ...
    [UpdateInGroup(typeof(PlayerSamplesSystemGroup))]
    ...
    public partial struct SpawnPlayerSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            for (var i = 0; i < connectionEntities.Length; i++)
            {
                ...
                Debug.Log(
                    $"[SpawnPlayerSystem][{state.WorldUnmanaged.Name}] Spawning player CC '{player.ToFixedString()}' (from prefab '{prefabName}') for {networkId.ToFixedString()}.");
